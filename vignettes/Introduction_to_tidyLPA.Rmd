...
We can also center or standardize the variables (to have grand mean equal to 0 and standard deviation eqoual t 1, respectively), with the `to_center` and `to_scale` arguments to `plot_profiles()`, i.e.:
...
There is a lot of output from the `estimate_profiles()` function. The easiest way to access it is by changing what is returned; by default, the function returns a `data.frame` with the variables used to create the profiles and the profile with the highest posterior probability (and the posterior probability for the profile):

```{r}
estimate_profiles(d, broad_interest, enjoyment, self_efficacy, model = 1, n_profiles = 4)
```

We can change the `to_return` argument to "mclust" to instead return the full output from the estimation:
    
```{r}
m3_mclust <- estimate_profiles(d, broad_interest, enjoyment, self_efficacy, model = 1, n_profiles = 4, to_return = "mclust")
```
...
```{r}
m3_processed_raw <- estimate_profiles(d, broad_interest, enjoyment, self_efficacy, model = 1, n_profiles = 4, center_raw_data = TRUE, scale_raw_data = TRUE)
```
...
```{r}
m3_mclust <- estimate_profiles(d, broad_interest, enjoyment, self_efficacy, model = 1, n_profiles = 4, return_orig_df = TRUE)
```
...
```{r, eval = F}
estimate_profiles(d, broad_interest, enjoyment, self_efficacy, model = 1, n_profiles = 4, prior_control = TRUE)
```
