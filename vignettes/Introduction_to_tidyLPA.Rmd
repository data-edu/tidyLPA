...
Note that you may wish to identify (and remove) univariate and multivariate outliers. See the functions [here](https://gist.github.com/jrosen48/094eee8627be43b167c7c79388027174) for some exampels. 
...
```{r}
m3 <- estimate_profiles(d[, c("broad_interest", "enjoyment", "self_efficacy")], 
                        n_profiles = 4)
m3
```
...
```{r, eval = F}
estimate_profiles(d[, c("broad_interest", "enjoyment", "self_efficacy")], 
                  variances = "varying",
                  covariances = "varying",
                  n_profiles = 3)
```
...
Other options include how the raw data is processed. We can center or scale the data before estimating the profiles with the `scale` function:
