...
plot_profiles <- function(x, to_center = F, to_scale = F, plot_what = "tibble", 
                          ...
                          ci = .95) {
  if (plot_what == "tibble") { # seems like you could have it detect this and just adjust automatically
    ...
    if (plot_error_bars == TRUE) {
      ...
      x %>%
        ...
        ggplot(aes_string(x = "profile", # These will probably have to be updated soon with tidyeval
                          y = "mean", 
                          fill = "key", 
                          ymin = "ymin", 
                          ymax = "ymax")) +
    ...
    plotdat <- reshape( # woah! Old school. : )
      plotdat,
      direction = "long",
    ...
    if (!is.null(ci)) {
      ...
      ses <- data.frame(apply(bootstraps$mean, 3, function(class) {
        apply(class, 2, quantile, probs = c((.5 * (1 - ci)), 
                                            1 - (.5 * (1 - ci))))
      })) # throughout this whole section there's some code styling like this that is different than you've used in the past. I'd recommend trying to stay consistent.
      if (to_center) {
